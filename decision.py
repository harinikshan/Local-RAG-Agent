...
def next_step(
    ...
    if tcs:
        ...
        # Intercept tool call loops (e.g. under less capable failover models)
        is_dup = False
        for h in history:
            if h.get("kind") == "action" and h.get("tool") == name:
                if h.get("arguments") == args:
                    is_dup = True
                    break
        
        if is_dup:
            print(f"[decision] loop detected for {name}({json.dumps(args)[:60]}) — forcing synthesis")
            reply = LLM().chat(
                prompt=prompt + "\n\nCRITICAL: You are repeating a tool call that already has a result. Do NOT call any more tools. Answer the goal directly using the ATTACHED ARTIFACTS or MEMORY HITS above.",
                system=SYSTEM,
                cache_system=True,
                tools=None,
                tool_choice=None,
